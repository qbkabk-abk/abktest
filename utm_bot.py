...
Features:
  - Allow list (only approved Telegram user IDs can use the bot)
...
# ── Allow List ───────────────────────────────────────────────────────────────
# Add Telegram user IDs here. Send /myid to the bot to get your ID.
...
def extract_yt_handle(raw: str) -> str:
    """Parse YouTube handle from @handle, URL, or plain text."""
    raw = raw.strip()
...
async def start(update: Update, context) -> int:
    """Entry point — check access, then ask for page URL."""
    user_id = update.effective_user.id
